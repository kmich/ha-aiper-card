...
`,Tt=["roller_brush","micromesh_filter","caterpillar_tread","propeller"];let zt=class extends dt{constructor(){super(...arguments),this._resolveKey=""}static async getConfigElement(){return await Promise.resolve().then(function(){return Wt}),document.createElement(xt)}static getStubConfig(){return{type:`custom:${bt}`}}setConfig(t){if(!t)throw new Error("Invalid configuration");if(!t.device&&!t.entity&&!t.entities)throw new Error("Set a `device:` (or an `entity:` / `entities:` override)");this._config={show_footer:!0,...t},this._resolveKey=""}getCardSize(){return 4}updated(){if(!this.hass||!this._config)return;const t=`${this._config.device??""}|${this._config.entity??""}|${JSON.stringify(this._config.entities??{})}`;t!==this._resolveKey&&(this._resolveKey=t,this._resolve())}async _resolve(){this.hass&&this._config&&(this._resolved=await Ut(this.hass,{device:this._config.device,anchorEntity:this._config.entity,keys:Mt,domains:Pt,overrides:this._config.entities}))}_state(t){const e=this._resolved?.map[t];return e?this.hass?.states[e]:void 0}_moreInfo(t){const e=this._resolved?.map[t];e&&s(this,"hass-more-info",{entityId:e})}_select(t,e){const i=this._resolved?.map[t];i&&this.hass&&this.hass.callService("select","select_option",{entity_id:i,option:e})}_toggleRunning(t){const e=this._resolved?.map.running_switch;e&&this.hass&&this.hass.callService("switch",t?"turn_on":"turn_off",{entity_id:e})}_press(t){const e=this._resolved?.map[t];e&&this.hass&&this.hass.callService("button","press",{entity_id:e})}render(){if(!this.hass||!this._config)return Z;const t=this._state("status");if(!this._resolved)return K`<ha-card><div class="unavailable">Loading…</div></ha-card>`;if(!t)return K`<ha-card
    ...
    `}_renderConsumables(){if(!1===this._config?.show_consumables)return Z;const t=this._resolved?.deviceName,e=Tt.map(e=>{const i=this._state(e);if(!i||isNaN(Number(i.state)))return null;const s=Math.max(0,Math.min(100,Number(i.state)));let r=i.attributes.friendly_name??e.replace(/_/g," ");return t&&r.startsWith(`${t} `)&&(r=r.slice(t.length+1)),K`<div class="wear" @click=${()=>this._moreInfo(e)}>
    ...
    `:Z}};zt.styles=Ht,t([ft({attribute:!1})],zt.prototype,"hass",void 0),t([gt()],zt.prototype,"_config",void 0),t([gt()],zt.prototype,"_resolved",void 0),zt=t([ut(bt)],zt),window.customCards=window.customCards||[],window.customCards.push({type:bt,name:"Aiper Cleaner Card",description:"Status, battery, mode and clean-path controls for an Aiper pool cleaner.",preview:!0,documentationURL:"https://github.com/kmich/ha-aiper-card"}),console.info("%c AIPER-CARD %c 0.1.0 ","background:#039be5;color:#fff","color:#039be5");const It=["ph","orp","chlorine"],Lt={ph:"pH",orp:"ORP",chlorine:"Chlorine",tds:"TDS",ec:"EC"};let jt=class extends dt{constructor(){super(...arguments),this._resolveKey=""}static async getConfigElement(){return await Promise.resolve().then(function(){return Wt}),document.createElement(xt)}static getStubConfig(){return{type:`custom:${wt}`}}setConfig(t){if(!t)throw new Error("Invalid configuration");if(!t.device&&!t.entity&&!t.entities)throw new Error("Set a `device:` (or an `entity:` / `entities:` override)");this._config={show_score:!0,show_meta:!0,...t},this._resolveKey=""}getCardSize(){return 3}updated(){if(!this.hass||!this._config)return;const t=`${this._config.device??""}|${this._config.entity??""}|${JSON.stringify(this._config.entities??{})}`;t!==this._resolveKey&&(this._resolveKey=t,this._resolve())}async _resolve(){this.hass&&this._config&&(this._resolved=await Ut(this.hass,{device:this._config.device,anchorEntity:this._config.entity,keys:Nt,overrides:this._config.entities}))}_state(t){const e=this._resolved?.map[t];return e?this.hass?.states[e]:void 0}_moreInfo(t){const e=this._resolved?.map[t];e&&s(this,"hass-more-info",{entityId:e})}render(){if(!this.hass||!this._config)return Z;if(!this._resolved)return K`<ha-card><div class="unavailable">Loading…</div></ha-card>`;const t=["ph","orp","ec","tds","chlorine","score"].some(t=>this._state(t));if(!t)return K`<ha-card
        ><div class="unavailable">
          No Aiper water-quality entities found. Check the <code>device</code> in the card config.
        </div></ha-card
      >`;const e=this._config.name||this._resolved.deviceName||this._state("ph")?.attributes.friendly_name||"Aiper Water Quality",i=this._config.gauges?.length?this._config.gauges:It;return K`
      <ha-card>
    ...
    `}_renderGauge(t){const e=this._state(t);if(!e||isNaN(Number(e.state)))return Z;const i=Number(e.state),s=Ot[t],r=e.attributes.unit_of_measurement??s?.unit??"",o=s?.min??0,n=s?.max??100,a=Math.max(0,Math.min(1,(i-o)/(n-o||1)));let c="var(--success-color, #43a047)";s&&(i<s.warn[0]||i>s.warn[1]?c="var(--error-color, #db4437)":(i<s.good[0]||i>s.good[1])&&(c="var(--warning-color, #ffa600)"));const l=Math.PI*(1-a),h=38+30*Math.cos(l),d=40-30*Math.sin(l);return K`
    ...
    `}_rel(t){const e=Date.parse(t);if(isNaN(e))return t;const i=Math.round((Date.now()-e)/1e3);return i<90?"just now":i<5400?`${Math.round(i/60)} min ago`:i<172800?`${Math.round(i/3600)} h ago`:`${Math.round(i/86400)} d ago`}};jt.styles=Ht,t([ft({attribute:!1})],jt.prototype,"hass",void 0),t([gt()],jt.prototype,"_config",void 0),t([gt()],jt.prototype,"_resolved",void 0),jt=t([ut(wt)],jt),window.customCards=window.customCards||[],window.customCards.push({type:wt,name:"Aiper Water Quality Card",description:"pH, ORP, chlorine and score gauges for an Aiper HydroComm monitor.",preview:!0,documentationURL:"https://github.com/kmich/ha-aiper-card"});let Dt=class extends dt{constructor(){super(...arguments),this._computeLabel=t=>({device:"Aiper device",name:"Name (optional)",image:"Header image URL (optional)",gauges:"Chemistry gauges",show_mode:"Show mode selector",show_clean_path:"Show clean-path selector",show_consumables:"Show consumable wear",show_footer:"Show footer",show_score:"Show score ring",show_meta:"Show temperature / sample time",compact:"Compact (hide header image)"}[t.name]??t.name)}setConfig(t){this._config=t}get _isMonitor(){return(this._config?.type??"").includes(wt)}_schema(){const t=[{name:"device",selector:{device:{integration:At}}},{name:"name",selector:{text:{}}}];return this._isMonitor?[...t,{name:"gauges",selector:{select:{multiple:!0,mode:"list",options:[{value:"ph",label:"pH"},{value:"orp",label:"ORP"},{value:"chlorine",label:"Chlorine"},{value:"tds",label:"TDS"},{value:"ec",label:"EC"}]}}},{type:"grid",schema:[{name:"show_score",selector:{boolean:{}}},{name:"show_meta",selector:{boolean:{}}}]}]:[...t,{name:"image",selector:{text:{}}},{type:"grid",schema:[{name:"show_mode",selector:{boolean:{}}},{name:"show_clean_path",selector:{boolean:{}}},{name:"show_consumables",selector:{boolean:{}}},{name:"show_footer",selector:{boolean:{}}},{name:"compact",selector:{boolean:{}}}]}]}_valueChanged(t){this._config&&s(this,"config-changed",{config:{...this._config,...t.detail.value}})}render(){return this.hass&&this._config?K`
      <ha-form
        .hass=${this.hass}
        .data=${this._config}
        .schema=${this._schema()}
    ...
    `:Z}};t([ft({attribute:!1})],Dt.prototype,"hass",void 0),t([gt()],Dt.prototype,"_config",void 0),Dt=t([ut(xt)],Dt);var Wt=Object.freeze({__proto__:null,get AiperCardEditor(){return Dt}});export{zt as AiperCleanerCard,jt as AiperMonitorCard};
